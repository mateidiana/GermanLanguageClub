...
public class VocabularyExamService {
    ...
    public void createQuestion(int courseId, String question, String rightAnswer){
        ...
        VocabularyExam vocab=getVocabExamById(courseId);
    ...
    public List<Student> filterStudentsByBestGrade(int teacherId,int examId){
        idDataCheck(examId);
        if (getVocabExamById(examId).getTeacher()!=teacherId)
            throw new BusinessLogicException("You don't have access to this exam!");
        List<Student> bestGrade=new ArrayList<>();
        for (ExamResult result:examResultRepo.getAll())
            if (result.getResult()==10.0 && result.getExam()==examId)
                bestGrade.add(getStudentById(result.getStudent()));
        return bestGrade;
    }
